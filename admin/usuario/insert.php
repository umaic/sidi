...
$display_tr_org = "none";
$tipo_rel_tema = array(42);
$display_tr_tema = "none";
...
if (isset($_GET["accion"]) && $_GET["accion"] == "actualizar"){
	...
	if (!empty($usuario_vo->id_org))	$display_tr_org = '';
	if (!empty($usuario_vo->id_tema))	$display_tr_tema = '';
...
<form method="POST" onsubmit="submitForm(event);return false;">
  <table border="0" cellpadding="3" cellspacing="1" width="70%" align="center">
	  <tr>
	  	<td align="center"><img src="images/icono_enviar_email.jpg">&nbsp;<a href="#" onclick="window.open('send_email_activacion.php?to=<?=$usuario_vo->email?>&asunto=Confirmaci&oacute;n de activaci&oacute;n de usuario SIDI&from=<?=$quien->email?>&login=<?=$usuario_vo->login?>&pass=<?=$usuario_vo->pass?>&quien=<?=$quien->nombre?>','','top=20,left=20,width=650,height=650');">Enviar correo de confirmaci&oacute;n</a></td>
	  </tr>
	</table>
	<br>
	<table class="tabla_insertar">
		<tr>
			  <td align="right">Tipo de Usuario</td>
				<td>
				  <select name="id_tipo" class="select" onchange="setOrgTema(this.value)">
						<? $tipo_usuario_dao->ListarCombo('combo',$usuario_vo->id_tipo,''); ?>
					</select>
				</td>
			</tr>
		...
		<tr id="tr_org" style="display:<?=$display_tr_org?>">
			...
			<td>
				<select id="id_org" name="id_org" class="select">
                    ...
                    <? $org_dao->ListarCombo('combo',$usuario_vo->id_org,'id_tipo IN (1,2,3,4)'); ?>
				</select>
			</td>
		</tr>
		<tr id="tr_tema" style="display:<?=$display_tr_tema?>">
			<td align="right">Cluster</td>
			<td>
				<select id="id_tema" name="id_tema" class="select">
					<option value=0></option>
					<? $tema_dao->ListarCombo('combo',$usuario_vo->id_tema,'id_clasificacion=2'); ?>
				</select>
			</td>
		</tr>
		<tr>

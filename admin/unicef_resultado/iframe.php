...
<form method="POST" onsubmit="submitFormIframe(event,'<?=$id_node_papa?>');return false;">
	<table class="tabla_insertar">
        ...
        if ($accion == 'actualizar' && !$resultado_dao->checkForeignKeys($id)){
            ?>
            <tr><td align="right">
				<a class="boton_new" href='#' onclick="if(confirm('Est&aacute; seguro que desea borrar este Resultado?')){borrarRegistroIframe('UnicefResultadoDAO','<?=$id?>','<?=$id_node_papa?>')}else{return false};"><img src='/sissh/images/unicef/delete.gif' border='0' title='Borrar' />&nbsp;Borrar este Resultado</a>&nbsp;
            </td></tr>
        <? } ?>
		<tr>

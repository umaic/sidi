...
function enviar_formulario(input_hidden,input_text,multiple,combo_extra,id,nom){
	...
	if (multiple == 0){
	    ...
	    //NUMERO DE PERSONAS
	  	if (combo_extra != "")
			var num_per = prompt("Número de personas beneficiadas en esta población?",0);
		...
		if (in_array == 0){
			...
			if(!confirm('Población agregada, desea agrear otra?')){
			  window.close();
			}
		}
		else{
		  alert('Ya existe la Población');
		}
	}
}
</script>
			...
			<tr>
				<td>
					<table cellpadding="5" cellspacing="1"> 
						<tr><td><b>Opción 1</b>: </td><td align="left"><a href="buscar_poblacion.php?<?=$_SERVER["QUERY_STRING"]?>&op=1">Buscar Por Palabra Clave</a></td></tr>
						<tr><td><b>Opción 2</b>: </td><td align="left"><a href="buscar_poblacion.php?<?=$_SERVER["QUERY_STRING"]?>&op=2">Listar todas las Poblaciones</a></td></tr>
					</table>
				</td>
			</tr>
		</table>
						...
						<tr>
							...
							<td>
								<select id='criterio_col' name='criterio_col' class='select'>
								  <option value='NOM_POBLA_ES'>Nombre en Español</option>
								  <option value='NOM_POBLA_IN'>Nombre en Español</option>
								</select>
							</td>
						</tr>
						<tr><td>&nbsp;</td></tr>
		...
		if ($op == 1){
	    	...
	    	echo "<td valign='top'>
					<table width='700' align='center' cellspacing='1' cellpadding='3' class='tabla_consulta'>
	    				<tr class='titulo_lista'><td align='center' colspan='2'>Resultado de la búsqueda de Poblaciones por palabra</td></tr>
						<tr><td>Palabra: <b>".$criterio."</b> - Buscando en: <b>";
						if ($criterio_col == "NOM_POBLA_ES")	echo "Nombre en Español";
						else										echo "Nombre en Inglés";
						echo "</b></td></tr>
					</table>";
			echo "</td>";
			echo "</tr>";
			...
			echo "<tr>
				<td colspan='5'><b>Ordenar listado por</b>:
					<select id='col_orden' name='col_orden' class='select'>
						<option value='NOM_POBLA_ES' ".$sel_col_orden["NOM_POBLA_ES"].">Nombre en Español</option>
						<option value='NOM_POBLA_IN' ".$sel_col_orden["NOM_POBLA_IN"].">Nombre en Inglés</option>
					</select>&nbsp;
		...
		if ($num_arr > 0){
			...
			echo "<tr><td>&nbsp;</td></tr>";
		  	...
		  	if ($num_arr_total > $num_reg_pag){
				...
				echo "Ir a la página:&nbsp;<select onchange=\"location.href='buscar_poblacion.php?field_hidden=".$_GET["field_hidden"]."&field_text=".$_GET["field_text"]."&multiple=".$_GET["multiple"]."&combo_extra=".$_GET["combo_extra"]."&op=".$_GET["op"]."&col_orden=".$col_orden_url."&dir_orden=".$dir_orden_url."&tipo=".$tipo_url."&page='+this.value\" class='select'>";

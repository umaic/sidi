 ...
 * DAO de FuentePba
 *
 * Contiene los métodos de la clase FuentePba 
 * @author Ruben A. Rojas C.
 */

Class UnicefFuentePbaDAO {
	 ...
	 * @access public
	 */	
	function UnicefFuentePbaDAO (){
		$this->conn = MysqlDb::getInstance();
		$this->tabla = "unicef_fuente_pba";
		$this->columna_id = "id_fuente";
		$this->columna_nombre = "codigo";
		$this->columna_order = "codigo";
	}

	/**
	 * Consulta los datos de un FuentePba
	 * @access public
	 * @param int $id ID del FuentePba
	 * @return VO
	 */	
	function Get($id){
	...
	/**
	 * Lista los FuentePba que cumplen la condición en el formato dado
	 * @access public
	 * @param string $formato Formato en el que se listarán los FuentePba, puede ser Tabla o ComboSelect
	 * @param int $valor_combo ID del FuentePba que será selccionado cuando el formato es ComboSelect
	 * @param string $condicion Condición que deben cumplir los FuentePba y que se agrega en el SQL statement.
	 */			
	...
	/**
	 * Carga un VO de FuentePba con los datos de la consulta
	 * @access public
	 * @param object $vo VO de FuentePba que se va a recibir los datos
	 * @param object $Resultset Resource de la consulta
	 * @return object $vo VO de FuentePba con los datos
	 */			
	function GetFromResult ($vo,$Result){

		$vo->id = $Result->{$this->columna_id};
		$vo->nombre = $Result->{$this->columna_nombre};
		$vo->id_donante = $Result->id_donante;
	...
	function ListarTabla($condicion){
		...
		$layout->adminGrid( array ('nombre' => array ('titulo' => 'C&oacute;digo')), 
                            array('id_donante' => array('titulo' => 'Donante','dao' => 'UnicefDonanteDAO', 'nom' => 'nombre', 'filtro' => false))
                          );
	}

	/**
	 * Inserta un FuentePba en la B.D.
	 * @access public
	 * @param object $vo VO de FuentePba que se va a insertar
	 */		
	function Insertar($vo){
		//CONSULTA SI YA EXISTE
		$a = $this->GetAllArray($this->columna_nombre." = '".$vo->nombre."'");
		if (count($a) == 0){
			$sql =  "INSERT INTO ".$this->tabla." (".$this->columna_nombre.",id_donante) VALUES ('".$vo->nombre."',$vo->id_donante)";
			$this->conn->Execute($sql);
			echo "Registro insertado con &eacute;xito!";
		}
		else{
			echo "Error - Existe una Fuente con el mismo c&oacute;digo";
		}

	}

	/**
	 * Actualiza un FuentePba en la B.D.
	 * @access public
	 * @param object $vo VO de FuentePba que se va a actualizar
	 */		
	function Actualizar($vo){
		$sql =  "UPDATE ".$this->tabla." SET ";
		$sql .= $this->columna_nombre." = '".$vo->nombre."',";
        $sql .= "id_donante = $vo->id_donante";
	...
	/**
	 * Borra un FuentePba en la B.D.
	 * @access public
	 * @param int $id ID del FuentePba que se va a borrar de la B.D
	 */	
	function Borrar($id){
	...
	function checkForeignKeys($id){
		...
		$tabla_rel = 'unicef_producto_awp_fuente_pba';

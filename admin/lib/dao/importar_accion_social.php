...
$conn->Execute($sql);
$sql = "INSERT INTO ocha_sissh_despla_import.periodo (desc_perio,orden) SELECT desc_perio,orden FROM ocha_sissh.periodo";
$conn->Execute($sql);
...
if (isset($_GET["submit"])){
	...
	foreach($archivos as $f => $file){
        ...
        if ($next == 1 || $f == 0){
           $next = 0;
		   $next = $des_dao->ImportarAccionSocial($userfile,$id_clase,$id_tipo,$id_fuente,$accion,$f_corte);
        }
	}

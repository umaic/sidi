 ...
 * DAO de Indicador
 *
 * Contiene los métodos de la clase Indicador 
 * @author Ruben A. Rojas C.
 */

Class UnicefIndicadorDAO {
	 ...
	 * @access public
	 */	
	function UnicefIndicadorDAO (){
		$this->conn = MysqlDb::getInstance();
		$this->tabla = "unicef_indicador";
		$this->columna_id = "id_indicador";
		$this->columna_nombre = "nombre";
		$this->columna_order = "nombre";
	}

	/**
	 * Consulta los datos de un Indicador
	 * @access public
	...
	/**
	 * Lista los Indicador que cumplen la condición en el formato dado
	 * @access public
	 * @param string $formato Formato en el que se listarán los Indicador, puede ser Tabla o ComboSelect
	 * @param int $valor_combo ID del Indicador que será selccionado cuando el formato es ComboSelect
	 * @param string $condicion Condición que deben cumplir los Indicador y que se agrega en el SQL statement.
	 */			
	...
	/**
	 * Carga un VO de Indicador con los datos de la consulta
	 * @access public
	 * @param object $vo VO de Indicador que se va a recibir los datos
	 * @param object $Resultset Resource de la consulta
	 * @return object $vo VO de Indicador con los datos
	 */			
	function GetFromResult ($vo,$Result){

		$vo->id = $Result->{$this->columna_id};
		$vo->nombre = $Result->{$this->columna_nombre};
        $vo->resultado = $Result->resultado;
        $vo->producto_cpap = $Result->producto_cpap;
	...
	/**
	 * Inserta un Indicador en la B.D.
	 * @access public
	 * @param object $vo VO de Indicador que se va a insertar
	 */		
	function Insertar($vo){
		//CONSULTA SI YA EXISTE
		$a = $this->GetAllArray($this->columna_nombre." = '".$vo->nombre."'");
		if (count($a) == 0){
			$sql =  "INSERT INTO ".$this->tabla." (".$this->columna_nombre.",resultado,producto_cpap) VALUES ('".$vo->nombre."',$vo->resultado,$vo->producto_cpap)";
			$this->conn->Execute($sql);
			echo "Registro insertado con &eacute;xito!";
		}
		else{
			echo "Error - Existe un Indicador con el mismo nombre";
		}

	}

	/**
	 * Actualiza un Indicador en la B.D.
	 * @access public
	 * @param object $vo VO de Indicador que se va a actualizar
	 */		
	function Actualizar($vo){
		$sql =  "UPDATE ".$this->tabla." SET ";
		$sql .= $this->columna_nombre." = '".$vo->nombre."',";
        $sql .= "resultado = $vo->resultado,";
        $sql .= "producto_cpap = $vo->producto_cpap";
	...
	/**
	 * Borra un Indicador en la B.D.
	 * @access public
	 * @param int $id ID del Indicador que se va a borrar de la B.D
	 */	
	function Borrar($id){
	...
	function checkForeignKeys($id){
		...
		$tabla_rel = 'unicef_resultado_cpd_indicador';

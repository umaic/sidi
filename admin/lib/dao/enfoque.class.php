 ...
 * DAO de Enfoque
 *
 * Contiene los métodos de la clase Enfoque
...
Class EnfoqueDAO {
	...
	function EnfoqueDAO (){
		...
		$this->tabla = "enfoque";
		$this->columna_id = "ID_ENF";
		$this->columna_nombre = "NOM_ENF_ES";
		$this->columna_order = "NOM_ENF_ES";
	}

	/**
	 * Consulta los datos de una Enfoque
	 * @access public
	 * @param int $id ID del Enfoque
	 * @return VO
	 */
	function Get($id){
		...
		//Carga el VO
		$enfoque_vo = $this->GetFromResult($enfoque_vo,$row_rs);
	...
	/**
	 * Lista los Enfoque que cumplen la condición en el formato dado
	 * @access public
	 * @param string $formato Formato en el que se listarán los Enfoque, puede ser Tabla o ComboSelect
	 * @param int $valor_combo ID del Enfoque que será selccionado cuando el formato es ComboSelect
	 * @param string $condicion Condición que deben cumplir los Enfoque y que se agrega en el SQL statement.
	 */
	...
	/**
	 * Lista los Enfoque en una Tabla
	 * @access public
	 */
	...
	function Reportar(){
		...
		echo "<table align='center' width='500' cellspacing='1' cellpadding='3'>
			<tr><td>&nbsp;</td></tr>
			<tr class='titulo_lista'><td align='center' colspan=4><b>ENFOQUES EN EL SISTEMA</b></td></tr>
			<tr><td>&nbsp;</td></tr>";
	...
	/**
	 * Carga un VO de Enfoque con los datos de la consulta
	 * @access public
	 * @param object $vo VO de Enfoque que se va a recibir los datos
	 * @param object $Resultset Resource de la consulta
	 * @return object $vo VO de Enfoque con los datos
	 */
	function GetFromResult ($vo,$Result){

		$vo->id = $Result->{$this->columna_id};
		$vo->nombre_es = $Result->{$this->columna_nombre};
		$vo->nombre_in = $Result->NOM_ENF_IN;

		return $vo;
	}

	/**
	 * Inserta un Enfoque en la B.D.
	 * @access public
	 * @param object $enfoque_vo VO de Enfoque que se va a insertar
	 */
	function Insertar($enfoque_vo){
		//CONSULTA SI YA EXISTE
		$cat_a = $this->GetAllArray($this->columna_nombre." = '".$enfoque_vo->nombre_es."'");
		if (count($cat_a) == 0){
			$sql =  "INSERT INTO ".$this->tabla." (".$this->columna_nombre.",NOM_ENF_IN) VALUES ('".$enfoque_vo->nombre_es."','".$enfoque_vo->nombre_in."')";
	...
	/**
	 * Actualiza un Enfoque en la B.D.
	 * @access public
	 * @param object $enfoque_vo VO de Enfoque que se va a actualizar
	 */
	function Actualizar($enfoque_vo){
		$sql =  "UPDATE ".$this->tabla." SET ";
		$sql .= $this->columna_nombre." = '".$enfoque_vo->nombre_es."',";
		$sql .= "NOM_ENF_IN = '".$enfoque_vo->nombre_in."'";

		$sql .= " WHERE ".$this->columna_id." = ".$enfoque_vo->id;
	...
	/**
	 * Borra un Enfoque en la B.D.
	 * @access public
	 * @param int $id ID del Enfoque que se va a borrar de la B.D
	 */
	function Borrar($id){

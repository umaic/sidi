 ...
 * DAO de Funcionario
 *
 * Contiene los métodos de la clase Funcionario 
 * @author Ruben A. Rojas C.
 */

Class UnicefFuncionarioDAO {
	 ...
	 * @access public
	 */	
	function UnicefFuncionarioDAO (){
		$this->conn = MysqlDb::getInstance();
		$this->tabla = "unicef_funcionario";
		$this->columna_id = "id_funcionario";
	 ...
	 * @access public
	 * @param int $id ID del Funcionario
	 * @return VO
	 */	
	function Get($id){
	...
	/**
	 * Lista los Funcionario que cumplen la condición en el formato dado
	 * @access public
	 * @param string $formato Formato en el que se listarán los Funcionario, puede ser Tabla o ComboSelect
	 * @param int $valor_combo ID del Funcionario que será selccionado cuando el formato es ComboSelect
	 * @param string $condicion Condición que deben cumplir los Funcionario y que se agrega en el SQL statement.
	 */			
	...
	/**
	 * Carga un VO de Funcionario con los datos de la consulta
	 * @access public
	 * @param object $vo VO de Funcionario que se va a recibir los datos
	 * @param object $Resultset Resource de la consulta
	 * @return object $vo VO de Funcionario con los datos
	 */			
	function GetFromResult ($vo,$Result){

		$vo->id = $Result->{$this->columna_id};
		$vo->nombre = $Result->{$this->columna_nombre};
		$vo->apellido = $Result->apellido;
	...
	/**
	 * Inserta un Funcionario en la B.D.
	 * @access public
	 * @param object $vo VO de Funcionario que se va a insertar
	 */		
	function Insertar($vo){
		//CONSULTA SI YA EXISTE
		$a = $this->GetAllArray($this->columna_nombre." = '".$vo->nombre."' AND apellido = '$vo->apellido'");
		if (count($a) == 0){
			$sql =  "INSERT INTO ".$this->tabla." (".$this->columna_nombre.",apellido) VALUES ('".$vo->nombre."','$vo->apellido')";
			$this->conn->Execute($sql);
			echo "Registro insertado con &eacute;xito!";
		}
		else{
			echo "Error - Existe un funcionario con el mismo nombre";
		}

	}

	/**
	 * Actualiza un Funcionario en la B.D.
	 * @access public
	 * @param object $vo VO de Funcionario que se va a actualizar
	 */		
	function Actualizar($vo){
		
        $sql =  "UPDATE ".$this->tabla." SET ";
		$sql .= $this->columna_nombre." = '".$vo->nombre."',";
		$sql .= " apellido = '".$vo->apellido."'";
	...
	/**
	 * Borra un Funcionario en la B.D.
	 * @access public
	 * @param int $id ID del Funcionario que se va a borrar de la B.D
	 */	
	function Borrar($id){
	...
	function checkForeignKeys($id){
		...
		$tabla_rel_1 = 'unicef_convenio';
		$col_id = $this->columna_id;
		
		$sql = "SELECT ((SELECT count($col_id) FROM $tabla_rel WHERE $col_id = $id) + (SELECT count($col_id) FROM $tabla_rel_1 WHERE $col_id = $id)) AS total";
		$rs = $this->conn->OpenRecordset($sql);

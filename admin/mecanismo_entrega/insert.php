...
if (isset($_GET["accion"]) && $_GET["accion"] == "actualizar"){
    $id = $_GET["id"];
    $mecanismo_entrega_vo = $mecanismo_entrega_dao->Get($id);
}
...
<form method="POST" onsubmit="submitForm(event);return false;">
    <table class="tabla_insertar">
        <tr><td align="right">Nombre</td><td><input type="text" id="nombre" name="nombre" size="40" value="<?=$mecanismo_entrega_vo->nombre;?>" class="textfield" /></td></tr>
        <tr>
            <td colspan="2" align='center'>
                <br>
                <input type="hidden" name="accion" value="<?=$accion?>" />
                <input type="hidden" name="id" value="<?=$mecanismo_entrega_vo->id;?>" />

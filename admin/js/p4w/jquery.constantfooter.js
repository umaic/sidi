(function($)
{
	...
	$.fn.constantfooter = function (settings) {
		...
		return this.each(function () {
            ...
            // Show a minimize button if required
			if (config.showminimize) {
				$(this).prepend("<div style=\"float: right;margin-right:10px\" class=\"" + classModifier + "minimize\"></div>");
				$("." + classModifier + "minimize").css({ cursor: "pointer" });
				$("." + classModifier + "minimize").click( function () {
					$(this).parent().toggleClass('constantfooterminimized');
                    $j(this).toggleClass('constantfootermaximize');

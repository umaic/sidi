				...
				this.getClick_Y = function(p_event)
				{
				 ...
				 // Función para Identify = info al hacer click
				  this.identify = function(event){
					...
					//Si no es caso perfil, ubica el div de info en la posicion del click del mouse					
					if (args.indexOf('perfil') == -1){
						eval("document.getElementById('div_info').style.display='';document.getElementById('div_info').style.left=tempX;document.getElementById('div_info').style.top=tempY;");
					}
					else{
						eval("document.getElementById('div_info').style.display='block';");
					}
					
					eval("getDataV1('','consulta/mapserver_query.php?' + args +'&x_real='+x1+'&y_real='+y1,'div_info')");
				  }
				  
				//Muestra toda la información
				this.showAllInfo = function(event){
					eval("document.getElementById('div_all_info').style.display='block';");
					eval("getDataV1('','consulta/mapserver_query.php?show_all_info=1','div_all_info')");
				}
				
 				// Download la imagen
				this.download = function(event){
						eval("location.href='consulta/mapserver_download_img.php';")
				}

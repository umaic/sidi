...
pixel_img.src = '/sissh/images/mscross/pixel.gif';
...
function msMap(DivTag, ControlType, p_protocol)
{
	...
	var _iconLoading = '/sissh/images/mscross/loading_ind.gif';
	...
	this.setActionNone = function(){
		...
		_tagMap.style.cursor = "";
				...
				this.mapLoaded = function()
				{
					...
					//Asocia el map a la imagen
					if (_control == 'consulta_depto')	_tagMap.useMap = '#map_deptos';
					else if (_control == 'consulta_mpio')	_tagMap.useMap = '#map_mpios';
				...
				this.init = function()
				{
					...
					_tagReference.style.background = '#0066ff';
					...
					_tagZoombox.style.background = '#0066FF';	         //'#f0f0f0';
					...
					//Coloca la imagen de loading en el centro si no es el mapa de alta resolucion
					if (location.href.indexOf('_print') == -1){
						_tagLoading.style.left = (_map_w - 170) / 2 + 'px';
						_tagLoading.style.top  = (_map_h - 122) / 2 + 'px';
					}
					else{
						_tagLoading.style.left = 50 + 'px';
						_tagLoading.style.top  = 50 + 'px';
					}
					...
					_tagEvents.appendChild(_tagMap_B);  //Aqui coloca el mapa en el elemento <div>
				...
				this.getClick_Y = function(p_event)
				{
				 ...
				 // Función para Identify = info al hacer click
				  this.identify = function(event){
				    ...
				    var x1 = i.xPixel2Real(x);
				    var y1 = i.yPixel2Real(y);
					...
					//XY del click en la pantalla para mostrar el popup
                    ...
                    eval("document.getElementById('div_info').style.display='block';");
					eval("getDataV1('','consulta/unicef_mapserver_query.php?' + args +'&x_real='+x1+'&y_real='+y1,'div_info')");
				  }
				  
				//Muestra toda la información
				this.showAllInfo = function(event){
					eval("document.getElementById('div_all_info').style.display='block';");
					eval("getDataV1('','consulta/unicef_mapserver_query.php?show_all_info=1','div_all_info')");
				}
				
 				// Download la imagen
				this.download = function(event){
						eval("location.href='consulta/unicef_mapserver_download_img.php';")
				}
 				
				// Exportar a hoja de calculo
				this.exportCalc = function(event){
					args = i.get_map_url();
					url_tmp = args.split("?");
					
					args = url_tmp[1];
					
					case_url = getUrlParameter(args,'case');
					
					eval("location.href='export_data.php?case=mapa_export_xls&nombre_archivo=' + case_url + '_sissh';")
				}
				 
				//
				this.setZoomPanUrl = function (){
						//Agrega al url la variable para saber que es zoom
						if (_tagMap.src.indexOf('zoom_pan') == -1){
							_tagMap.src += '&zoom_pan=1';
						}
				} 
				...
				this.setOverlayPoints = function(p, p_icon, p_infoSkin)
				{
					...
					p_infoSkin = new msInfoSkin( '/sissh/images/mscross/angolo_a.png', '/sissh/images/mscross/angolo_b.png',
					'/sissh/images/mscross/angolo_c.png', '/sissh/images/mscross/angolo_d.png',
					'/sissh/images/mscross/report_t.png', '/sissh/images/mscross/report_d.png',
					'/sissh/images/mscross/report_l.png', '/sissh/images/mscross/report_r.png',
					'/sissh/images/mscross/report_x.png', '/sissh/images/mscross/close.png',
					'/sissh/images/mscross/report_arrow.png' );
					}
...
function pointOverlay( p_icon, p_infoSkin, p_title, p_x, p_y, p_item_name, p_item_value )
{
	...
	{
		// Create a default Info-window icon object...
		_infoSkin = new msInfoSkin( '/sissh/images/mscross/angolo_a.png', '/sissh/images/mscross/angolo_b.png',
		'/sissh/images/mscross/angolo_c.png', '/sissh/images/mscross/angolo_d.png',
		'/sissh/images/mscross/report_t.png', '/sissh/images/mscross/report_d.png',
		'/sissh/images/mscross/report_l.png', '/sissh/images/mscross/report_r.png',
		'/sissh/images/mscross/report_x.png', '/sissh/images/mscross/close.png',
...
function msIcon( p_img, p_shd, p_offsetX, p_offsetY )
{
	...
	if ( _img_name == null )
	{
		_img_name = '/sissh/images/mscross/mm_20_red.png';
		_shd_name = '/sissh/images/mscross/mm_20_shadow.png';
		_offsetX  = 6; _offsetY = 19;
	}
	if (_img_name == '') {_img_name='/sissh/images/mscross/pixel.gif';}
	if (_shd_name == '') {_shd_name='/sissh/images/mscross/pixel.gif';}
...
function msToolbar(p_msMap, _control, _default)
{
	...
	var _iconFullExtentButton = '/sissh/images/mscross/alpha_button_fullExtent.png';
	var _iconZoomboxButton    = '/sissh/images/mscross/alpha_button_zoombox.png';
	var _iconPanButton        = '/sissh/images/mscross/alpha_button_pan.png';
	var _iconZoominButton     = '/sissh/images/mscross/alpha_button_zoomIn.png';
	var _iconZoomoutButton    = '/sissh/images/mscross/alpha_button_zoomOut.png';
	
	//Extras Iconos
	var _iconIdentifyButton    = '/sissh/images/mscross/alpha_button_identify.png';
	var _iconDownloadButton    = '/sissh/images/mscross/save.png';
	var _iconExportCalcButton    = '/sissh/images/mscross/ooo-calc.png';
	var _iconShowAllInfoButton    	= '/sissh/images/mscross/all_info.png';
	...
	if (_default == true)
	{
		...
		var t_fullext = new msTool('Mapa Completo', _msMap.fullExtent, _iconFullExtentButton);
		var t_pan     = new msTool('Pan', _msMap.setActionPan, _iconPanButton, function(e, map, x, y){map.setZoomPanUrl();map.dragStart(e);});
		...
		//Extras Iconos
		var t_identify = new msTool('Información Municipio', _msMap.setActionZoombox, _iconIdentifyButton, function(e, map, x, y){map.identify(e);});
		var t_download = new msTool('Descargar Imagen',function(e, map, x, y){map.download();}, _iconDownloadButton, '');
		var t_export_calc = new msTool('Exportar a Hoja de Calculo',function(e, map, x, y){map.exportCalc();}, _iconExportCalcButton, '');
		var t_show_all_info = new msTool('Información Completa',function(e, map, x, y){map.showAllInfo();}, _iconShowAllInfoButton, '');
		var t_mouse_over_info = new msTool('Mouse Over Info',function(e, map, x, y){map.mouseOverInfo();}, _iconIdentifyButton, '');

		//Standard se usa para la consulta en mapa.php
		if (_control == 'standard'){ 
			...
			//Extras Iconos
			this.addMapTool(t_identify);
			this.addMapTool(t_show_all_info);
			this.addMapTool(t_download);
			this.addMapTool(t_export_calc);
		}
		//perfil se usa en mapa_perfil.php
		else if (_control == 'perfil'){
			//Extras Iconos
			this.addMapTool(t_identify);
			//this.addMapTool(t_export_calc);
		...
		//consulta se usa en mapa_consulta.php
		/*else if (_control == 'consulta_mpio'){
	...
	if (_control != 'consulta_mpio' || _control != 'consulta_depto'){
		for (i=0; i<_toolbarArray.length; i++)
		{

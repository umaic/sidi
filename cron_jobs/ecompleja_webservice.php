...
// Fecha inicial
//$f_ini = (!empty($_GET['f_ini'])) ? $_GET['f_ini'] : '2007-1-1' ;
//$f_fin = (!empty($_GET['f_fin'])) ? $_GET['f_fin'] : false ;

if (empty($_GET['id'])) {
    die('Falta parametros papá!');   
}
else {
    $sync_eve_id = $_GET['id'];
    //$sync_eve_id = 46990;  // Id de eventos desde 1-Feb-2013, se usa esto, porque se esta modificando eventos pasados y la sync debe tomar esos cambios
    //$sync_eve_id = 38610;  // Id de eventos desde 1-Ene-2012, revision total con updates el 2012
}
...
/*
if (empty($f_fin)) {
    $cf = "fecha_reg_even >= '$f_ini'";
}
else {
    $cf = "fecha_reg_even BETWEEN '$f_ini' AND '$f_fin'";
}

 */

$sql .= " WHERE c.id_even > $sync_eve_id AND LENGTH(sintesis_even) > 0";
...
while ($evento = $conn->FetchObject($rs)){
    ...
    if (strlen($sintesis) > 5){
        ...
        if (isset($fuente['desc'][0]) && strlen($fuente['desc'][0]) > 10){
            ...
            $descripcion = win_replace($fuente['desc'][0]);
        }
        else
        ...
        //Fuentes
	    $fuentes = $evento_dao->getFuenteEvento($id_evento);
        ...
        if (count($id_subcats) > 0){
            ...
            foreach ($id_muns as $id_mun){
                ...
                */
                ...
                $incident['incident_source'] = utf8_encode($source);
                ...
                foreach ($id_subcats as $id_s){
                    ...
                    */
                    ...
                    // Actores
                    $sqla = "SELECT id_actor 
                             FROM actor_descevento 
                             JOIN descripcion_evento d USING (id_deseven) 
                             JOIN evento_c e USING(id_even) 
                             WHERE e.id_even = $id_evento AND id_scateven = $id_s";

                    $rsa = $conn->OpenRecordset($sqla);
                    $actor = array();
                    while ($rowa = $conn->FetchRow($rsa)) {
                        $actor[] = $rowa[0];
                    }
                    ...
                    $sql = "SELECT id_sexo,id_subetnia,id_subcondicion,id_estado,id_ocupacion,id_raned,cant_victima,id_condicion,id_edad,cant_victima
                            ...
                            JOIN descripcion_evento d USING (id_deseven) 
                            JOIN evento_c e USING(id_even) 
                            WHERE e.id_even = $id_evento AND d.id_scateven = $id_s AND cant_victima > 0";
                ...
                $news = array();
                foreach ($fuentes['id_fuente'] as $f => $id_fuente){
                    $news[] = array('id_fuente' => $id_fuente, 
                                    'id_subfuente' => $fuentes['id_sfuente'][$f],
                                    'desc' => utf8_encode($fuentes['desc'][$f]),
                                    'fecha' => utf8_encode($fuentes['fecha'][$f]),
                                    'medio' => utf8_encode($fuentes['medio'][$f]),
                                );

                }

                $json[] = array('incident' => $incident, 
                                'category' => $category, 
                                'victim' => $victim, 
                                'actor' => $actor,
                                'news' => $news);
...
//$fp = fopen('sidih_ecompleja.json', 'w+');

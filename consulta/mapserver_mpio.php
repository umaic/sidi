...
if ($print == 0){
	...
	$size_font_label = 1;  //Tamaño en pixeles
}
else{
...
if ($id_depto_filtro > 0){
    ...
    // Nombre como estan en sl shp
    $deptos_shp = array('05' => 'ANTIOQUIA',
                        '08' => 'ATLANTICO',
                        '11' => 'BOGOTA D.C.',
                        '13' => 'BOLIVAR',
                        '15' => 'BOYACA',
	...
	//Elimina tildes
	$depto_nombre = $deptos_shp[$id_depto_filtro];
...
if ($map_ref == 0 && $case != 'perfil'){
  ...
  switch ($case){
	...
	if ($variacion == 0){
		$salir = 0;
		while ($salir == 0){
			if (isset($valores_mpio[0]) && $valores_mpio[0] == 0){
				$draw_cero = 1;
	...
	for($i=1;$i<($numclass+1);$i++){
		...
		$j = $i - 1;
        $sl = ($print == 0 && $i < ceil($numclass/2)) ? false : true;
    ...
    // Guarda el png para usar en otras apps, i.e. ERF, guarda IRSH
    if ($case == 'dato_sectorial' && $id_dato == 588){
		//imagepng($image,$_SERVER['DOCUMENT_ROOT']."/sissh/wms/irsh.png");
    }
	...
	//CACHE
	if ($id_cache == 0 || !$use_cache){
        if (in_array($case,array('desplazamiento','org','dato_sectorial','proyecto_undaf'))){
